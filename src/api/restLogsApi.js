  ...
  endpoints: (builder) => ({
    getRestLogsByTrip: builder.query({
      ...
      providesTags: (result, error, tripId) =>
        result
          ? [
              ...result.map(({ _id }) => ({ type: "RestLog", id: _id })),
    ...
    endRestAndStartDrive: builder.mutation({
      ...
      invalidatesTags: (result, error, { rest_id }) => [
        { type: "RestLog", id: rest_id },
        "Trips",
        "DriveSessions",
        "DashboardStats",
      ],
...
export const { useGetRestLogsByTripQuery, useEndRestAndStartDriveMutation } =
  restLogsApi;
